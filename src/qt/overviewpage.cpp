...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::OPU)
    {
    }
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    CAmount opuAvailableBalance = balance - immatureBalance - nLockedBalance;

    // OPU Watch-Only Balance
    CAmount nTotalWatchBalance = watchOnlyBalance + watchUnconfBalance;
    CAmount nAvailableWatchBalance = watchOnlyBalance - watchImmatureBalance - nWatchOnlyLockedBalance;

    // OPU labels
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, opuAvailableBalance, false, BitcoinUnits::separatorAlways));
    ...
    // OPU Available
    bool showOPUAvailable = settingShowAllBalances || opuAvailableBalance != nTotalBalance;
    bool showWatchOnlyOPUAvailable = showOPUAvailable || nAvailableWatchBalance != nTotalWatchBalance;
    ui->labelBalanceText->setVisible(showOPUAvailable || showWatchOnlyOPUAvailable);
    ui->labelBalance->setVisible(showOPUAvailable || showWatchOnlyOPUAvailable);
    ui->labelWatchAvailable->setVisible(showOPUAvailable && showWatchOnly);

    // OPU Pending
    bool showOPUPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyOPUPending = showOPUPending || watchUnconfBalance != 0;
    ui->labelPendingText->setVisible(showOPUPending || showWatchOnlyOPUPending);
    ui->labelUnconfirmed->setVisible(showOPUPending || showWatchOnlyOPUPending);
    ui->labelWatchPending->setVisible(showOPUPending && showWatchOnly);
    ...
    // OPU Locked
    bool showOPULocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyOPULocked = showOPULocked || nWatchOnlyLockedBalance != 0;
    ui->labelLockedBalanceText->setVisible(showOPULocked || showWatchOnlyOPULocked);
    ui->labelLockedBalance->setVisible(showOPULocked || showWatchOnlyOPULocked);
    ui->labelWatchLocked->setVisible(showOPULocked && showWatchOnly);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("OPU")
    updateDisplayUnit();
}
