...
#if defined(NDEBUG)
#error "OpulenceCoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uopu) are considered zero fee (for relaying and mining)
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    } else if (nHeight > 950000) { ret = blockValue * 0.10; }
    return ret;
}
...
{
    RenameThread("opulencecoin-scriptch");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // OPU: We use certain sporks during IBD, so check to see if they are
